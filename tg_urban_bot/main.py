...
async def cmd_start(message: Message):
    ...
    if not (await check_user(user_id)):
        ...
        await message.answer(
            """Поздравляю, вы успешно прошли регистрацию! Теперь вам доступно 10 тестовых запросов, расширить их число вы сможете, оформив подписку."""
        )
    await message.answer(
...
async def set_command_desc(bot: Bot):
    ...
    await bot.set_my_description(
        """Привет, я ассистент aid! Я помогаю искать талантливых специалистов из мира дизайна и архитектуры. Просто напишите свой запрос, и я предложу вам подходящих специалистов."""
    )
